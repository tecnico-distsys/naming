...
public class ZKNaming {
	...
	private void connect () throws IOException, InterruptedException {
		...
		//CountDownLatch is used to stop (wait) the main process until the client connects with the ZooKeeper ensemble.
	...
	public void bind (ZKRecord record) throws ZKNamingException {
		...
		try {
			...
			if (stat == null) 
				zoo.create(record.getPath(), record.getURI().getBytes(), ZooDefs.Ids.OPEN_ACL_UNSAFE, CreateMode.PERSISTENT);
